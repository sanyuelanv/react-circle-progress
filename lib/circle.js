...
var Circle = function (_Component) {
  ...
  function Circle(props) {
    ...
    _this.showText = _this.props.showText == false ? _this.props.showText : true;
    _this.textStyle = _this.props.textStyle || { size: 12, color: "#555", type: 0 };
    _this.linecap = _this.props.linecap || "square";
    return _this;
  }
  ...
  _createClass(Circle, [{
    ...
    key: '_renderText',
    value: function _renderText(current) {
      if (!this.showText) return;
      var _textStyle = this.textStyle,
          color = _textStyle.color,
          size = _textStyle.size,
          type = _textStyle.type;

      var res = current;
      if (type == 0) {
        res = parseInt(current * 100) + "%";
      } else if (type == 1) {
        res = current;
      } else if (type == 2) {
        res = parseInt(current * 100);
      ...
      return React.createElement(
        'text',
        { x: '50%', y: '50%', dy: size / 2, textAnchor: 'middle', style: { fontSize: size + 'px', fill: color } },
        res
      );
    }
  }, {
